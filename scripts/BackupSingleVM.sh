...
backupVM() {
      if [ "$PWR" == "Powered off" ] ; then
          ...
          echo -e "   - $name has been started again (`date`)\n" >> $PATHLOG
      	  ...
      	  echo -e "   - $name is dying out, 10s before next step (`date`)\n" >> $PATHLOG
      	  sleep 10
          echo -e "   - $name is currently under backuping (`date`)\n" >> $PATHLOG
          path=`find / -name "$name.vmx"`
          cutedpath="${path%"${path#/*/*/*/*/}"}"
...
# TO CHANGE : -----------------------------------------------------------------------------
find /vmfs/volumes/5e566f71-06f2da78-82d5-441ea15ee924/backup* -mtime +60 -exec rm -rf {} \; # permit to delete all backup* folder > 60 Days
